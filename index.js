  ...
  async (req, res) => {
    try {
      ...
      const processedDeliveries = deliveries.map((delivery) => {
        ...
        if (goodsMasterFilePath) {
          goodsMaster.forEach((goodsDetails) => {
            // Match delivery ID with goodsMaster ID or another unique identifier
            // if (goodsDetails["ID"] === delivery["ID"]) {
              volume = parseFloat(goodsDetails["CFT"] || goodsDetails["Volume"]) || volume;
              weight = parseFloat(goodsDetails["Weight"] || goodsDetails["weight(kgs)"]) || weight;
            // }
          });
        }
        ...
        console.log(`Delivery ${delivery["ID"]} -----> ${multiplicationValue.toFixed(2)}`);
